...
* 정확한 원인 파악이 되지 않아서 0.3초 딜레이를 통하여 키패드 입력시 바로 통신하지 못하게 예외 처리 합니다. (아마도 github api를 빠르게 호출하는 것으 방지하지 않았을까? 추측함)
3. 디자인 패턴 개선
* 현재는 기능이 많지 않아서 가장 기본적인 MVVM 패턴을 이용하였지만, 상황예 따라서 디자인패턴 변경도 고려해야 합니다.
4. 테스트 시간 부족
